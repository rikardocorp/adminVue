  ...
  <b-form :id="'forms-'+ urlRest" class="col-md-9 col-lg-10 col-xl-7 m-auto pt-3">
    ...
    <div class="d-flex justify-content-center mb-2 mySwitch">
      <toggle-button :labels="{checked: 'DNI', unchecked: 'RUC'}" :color="{checked: 'rgb(239, 123, 34)', unchecked: 'rgb(181, 181, 181)'}"
                     ...
                     v-model="item.typeDocument" class="mr-2">
      </toggle-button>
    ...
    <b-form-group v-show="item['hasEmail']"
      ...
      <b-input-group>
        ...
        <b-form-input v-if="optInput['email'].input=='input-email'"
                      ...
                      @blur.native="searchUser(null, true)"
                      :placeholder="optInput['email'].placeholder+'..'"></b-form-input>
        <b-input-group-button>
          <b-btn :disabled="isLoading || restricted" variant="primary" @click="searchUser(null, true)"><i class="fa fa-search"></i></b-btn>
        </b-input-group-button>
      </b-input-group>
      <!-- ERROR MESSAGE-->
    ...
    <b-form-group v-for="(option, _index) in optInput" :key="_index" :label-sr-only="option.srOnly"
      ...
      <b-input-group v-if="option.input=='onlyMultiSelect'">
        <b-input-group-addon class="bg-primary"><i :class="'fa ' + option.icon"></i></b-input-group-addon>
        <only-multi-select class="special_radius" ref="localidad"
      ...
      <!-- ERROR MESSAGE-->
      <form-error :async="true" :data="$v.item[_index]? $v.item[_index] : {} "></form-error>
    </b-form-group>
...
<script>
  ...
  import {DATA_PURCHASER as _purchaser} from '../../../../data/dnSaleSpecial'
  ...
  export default {
    name: 'Purchaser',
    props: ['name', 'urlRest', 'item', 'horizontal', 'keyname', 'restricted'],
    mixins: [Mixin],
    components: {
      cSwitch,
      OnlyMultiSelect,
    ...
    computed: {
      ...
      maxDNIRUC () {
        return this.item.typeDocument === 1 ? 8 : 11
      }
    },
    validations () {
      let validateA
      if (this.item.hasEmail) {
        validateA = _purchaser.validate
      } else {
        validateA = _purchaser.validate2
      }

      validateA.item.dniRuc = {
        required,
        // numeric,
        // minLength: minLength(this.maxDNIRUC),
        // maxLength: maxLength(this.maxDNIRUC)
      }
      return validateA
    },
    watch: {
      'item.typeDocument' (newVal) {
        console.log('item.typeDocument')
        console.log(newVal)
        if (newVal == 1) {
          console.log('DNIRUC  1')
          this.setDatepickerDisabled(true)
          this.optInput.razonSocial.label = 'Nombre'
          this.optInput.dniRuc.label = 'DNI'
        } else {
          console.log('DNIRUC  0')
          this.setDatepickerDisabled(false)
          this.optInput.razonSocial.label = 'Razon Social'
      ...
      localidad (newVal, oldVal) {
        ...
        if (Array.isArray(newVal)) {
          if (typeof newVal[0] !== 'object') {
            this.$set(this.item, 'departamento', newVal[0])
            this.$set(this.item, 'provincia', newVal[1])
            this.$set(this.item, 'distrito', newVal[2])
            // this.item.provincia = newVal[1]
            // this.item.distrito = newVal[2]
          } else {
            this.$set(this.item, 'departamento', newVal[0] ? newVal[0].name : '')
            this.$set(this.item, 'provincia', newVal[1] ? newVal[1].name : '')
            this.$set(this.item, 'distrito', newVal[2] ? newVal[2].name : '')
    ...
    methods: {
      getRegionID () {
        let onlyMultiselect = this.$refs.localidad[0]
        console.log('getRegionID')
        console.log(onlyMultiselect)
        if (onlyMultiselect.xvalue[0]) {
          let localidad = onlyMultiselect.xvalue[0].id
          this.locationID = localidad
          console.log('getRegionID end')
          localStorage.setItem('locationID', JSON.stringify(localidad))
        }
      },
      defaultPurchaser () {
        this.birthDate = ''
        this.localidad = []
        this.optInput = _purchaser.input
        this.lCols = 3
        this.owner = null
        this.hiddenEmail = false
        this.searchUser(this.$store.state.user.data.email, false)
        this.dniRucAux = null
        this.locationID = null
        let onlyMultiselect = this.$refs.localidad[0].localValue = []
      },
      selectDate (pickDate) {
        let newDate = ''
        if (pickDate) {
      ...
      // dniRUC () {
      //   console.log('DNIRUC')
      //   const value = this.item.typeDocument === undefined ? this.item.typeDocument.value : this.dniRucAux
      //   console.log(value)
      //   if (value == 1) {
      //     console.log('DNIRUC  1')
      //     this.setDatepickerDisabled(true)
      ...
      searchUser (localEmail = null, hasEmail) {
        let email = (localEmail === null) ? this.item.email : localEmail
        console.log('EMAIL: ', email)
        console.log(localEmail, this.item)
        if (email !== '') {
          console.log('******** PURCHASER searchUser')
      ...
      setDatepickerDisabled (isDNI = true) {
        // DatePicker
        ...
        if (isDNI) {
          ...
          this.optInput.birthDate.params.disabled.from = new Date(year, month, day)
        }
        this.optInput.birthDate.params.disabled.to = new Date(1800, month, day)
        // this.birthDate = ''
      }
    },
    mounted () {
      let purchaser = JSON.parse(localStorage.getItem('purchaser'))
      console.log('-- SPECIAL PURCHASER --')
      ...
      if (purchaser) {
        ...
        if (date) this.birthDate = date
        console.log('PURCHASER MOUNTED')
        console.log(this.item)
        console.log(this.item.hasEmail)
        if (purchaser.hasEmail) this.searchUser(purchaser.email, purchaser.hasEmail)
      }

      // this.dniRucAux = purchaser.typeDocument
      // this.dniRUC()
