<template>
  <div class="col-md-10 m-auto pt-3 pb-4 myPoliza">
      ...
      <div class="card-ticket cardCenter">
        ...
        <div class="xcontent">
          ...
          <div v-if="!isClient" class="seat">
            <h2>{{ data.pay.item.validityStart ? data.pay.item.validityStart: 'xx/xx/xxxx' }}</h2>
    ...
    <div v-if="!isClient" class="container abstractPrice pt-3 pb-4">
      <div v-if="data.sale.item.discount > 0">
        <div class="row text-success">
          ...
          <div class="col-6">{{ data.vehicleType.item.price | currency }}</div>
        </div>
        <hr>
        <div class="row text-danger secundary">
          <div class="col-6">Descuento s/.</div>
          <div class="col-6">{{ data.pay.item.discount | currency }}</div>
        </div>
      </div>
    ...
    <div class="container abstractPrice pt-2 pb-2" style="background: #35a213">
      <div class="title text-center text-white">
        <span class="fa-1x">Monto a Cancelar s/.</span> <span class="h4">{{ data.pay.item.amount | currency }}</span>
        <!--<button @click="deletePay2">DELETE</button>-->
      </div>
    </div>
    ...
    <b-button v-else @click="deletePay2" :pressed="false" variant="danger"
              class="mr-1 mt-3 mb-0 btn-pay hvr-shadow-radial hvr-bob" style="background: rgb(241, 65, 64);">
      <i :class="{'fa fa-money': data.pay.item.paymentType===1, 'fa fa-credit-card':  data.pay.item.paymentType==0}" aria-hidden="true"></i> ANULAR PAGO
    </b-button>
    <div v-if="!isClient">

      <!-- PARA LA AFOCAT 6Y7 -->
      <div v-if="company && (company.id==6 || company.id==7)" class="text-center mt-3">
      ...
      <div v-if="data.payment.item.id!==undefined" class="ticket absolute cardWrap mx-auto pb-4 mt-3 postPay">
        <div class="row">
          ...
          <div @click="typePostPay=2" :class="{'col-4 text-center postPay-list': true, 'pick': typePostPay==2}">
            ...
            <p :class="{'text-info': typePostPay==2}">POLIZA</p>
          </div>
        </div>
        ...
        <div v-show="typePostPay==2" class="postPay-item">
          ...
          <p class="text-center pt-2">
            ...
            <span class="text-info" style="font-size: 1.2em; font-weight: bolder;">#{{ policy ? policy.number : '[PENDIENTE]'  }}</span> se anulara</p>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    name: 'Success',
    props: ['name', 'horizontal', 'data', 'item', 'policy', 'company'],
    components: {
      ToggleButton,
      Avatar,
      Multiselect
    ...
    computed: {
      ...
      amountFinal () {
        return this.data.vehicleType.item.price - this.data.pay.item.discount
      },
      path () {
...
//       8    afocat aqp: /sales/{id}/printpolicy
      ...
      async reAsignar () {
        ...
        console.log(this.insurancePolicy.params)
        const value = this.insurancePolicy.params.value
        if (value === '' || value === null) {
        ...
        const data = {
          ...this.data.sale.item,
          insurancePolicy: this.insurancePolicy.params.value,
        }
        this.$delete(data, 'id')
        this.$delete(data, 'date')
        this.$emit('manageSale', data, true)
