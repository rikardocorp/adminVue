<template>
  <b-form :id="'forms-'+ urlRest" class="col-sm-10 col-md-9 col-lg-8 col-xl-6 m-auto pt-3">
    <!--<pre>{{ item }}</pre>-->
...
<script>
  import {DATA_VEHICLETYPE as _vehicleType} from '../../../../data/dnSaleSpecial'
  import FormError from '../../../../components/FormError.vue'
  import Multiselect from 'vue-multiselect'

  export default {
    name: 'VehicleType',
    props: ['name', 'horizontal', 'data', 'item', 'urlRest', 'restricted', 'pickPolice'],
    components: {
      FormError,
      Multiselect
    },
    ...
    methods: {
      addTag (newTag) {
        ...
        if (key === 'brand') {
          ...
          tag = {
            vehicleBrand: this.item.brand ? this.item.brand.vehicleBrand : '',
            vehicleModel: newTag
          }
        }
      ...
      selectOption (selectedOption) {
        ...
        if (key === 'policyType') {
          if (selectedOption === null) {
            ...
            if (this.pickPolice && this.pickPolice.policyType == policyType) {
              this.item['insurancePolicy'] = this.pickPolice
            } else {
              this.item['insurancePolicy'] = ''
            }

            this.loadPolicies(policyType)
          }
    ...
    watch: {
      'item.insurancePolicy' (newVal) {
        if (newVal === null) {
          if (this.pickPolice !== null) {
            this.item.insurancePolicy = this.pickPolice
          }
        }
      },
      'item.brand' (newVal) {
        if (newVal === null) {
          ...
          let vehicleBrand = newVal.vehicleBrand
          if (newVal.id) {
            // this.item['vehicleType'] = ''
      ...
      'item.policyType' (newVal) {
        console.log('item.policyType')
        console.log(newVal)
        if (newVal === null) {
          this.optInput['insurancePolicy'].params.options = []
          this.item['insurancePolicy'] = ''
        } else {
          let policyType = newVal.id
          if (newVal.id) {
            this.loadPolicies(policyType)
            // let companyId = this.item.insuranceCompany.id
            // let myUserID = this.$store.state.user.data.id
            // this.item['vehicleType'] = ''
            // let url = 'insurancepolicies?insuranceCompanyId=' + companyId + '&sold=0&userId=' + myUserID + '&policyType=' + policyType
            // let url = this.optInput['insurancePolicy'].params.url + policyType
            // console.log('GET OPTIONS POLICY')
            // this.getOption(url, 'insurancePolicy')
          }
        }
      }
